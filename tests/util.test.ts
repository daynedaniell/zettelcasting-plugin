...
describe('normalizeForPublishing', () => {
  ...
  it('strips an html comment that is not a structure marker', () => {
    // Comments are invisible in the vault, so publishing them verbatim would
    // put a private annotation into a public feed.
    assert.equal(
      normalizeForPublishing('Before.\n\n<!-- a normal comment -->\n\nAfter.\n'),
      'Before.\n\nAfter.'
    );
  });

  it('strips an Obsidian %% comment, inline and on its own line', () => {
    assert.equal(
      normalizeForPublishing('Public. %%private aside%%'),
      'Public.'
    );
    assert.equal(
      normalizeForPublishing('Before.\n\n%%\nprivate block\n%%\n\nAfter.\n'),
  ...
  it('publishes no more than it counts', () => {
    // getWordCount has always excluded comments. Publishing them anyway made
    // the count in the modal disagree with what actually went out.
    const note = 'One two three. %%four five six%%';
    const published = normalizeForPublishing(note);
    assert.equal(published, 'One two three.');
    assert.equal(getWordCount(published), getWordCount(note));
  });

  it('never strips comments out of a code block', () => {
    // `%%` is a LaTeX comment and `<!-- -->` is ordinary HTML. A plain regex
    // sweep treats the span between two of them as one comment and deletes the
    // code in between.
    const html =
      'Intro.\n\n```html\n<div>\n<!-- keep this -->\n</div>\n```\n\nOutro.';
    assert.equal(normalizeForPublishing(html), html);

    const tex = 'Text.\n\n```tex\n%% a comment\nx = 1\n%% another\n```';
    assert.equal(normalizeForPublishing(tex), tex);
  });

  it('never strips comments out of an inline code span', () => {
    const note = 'Use `%%literal%%` to mark it.';
    assert.equal(normalizeForPublishing(note), note);
  });

  it('strips comments around code without touching the code', () => {
    assert.equal(
      normalizeForPublishing('%%todo%%\n\n`a %%b%% c`\n\n<!--note-->After.'),
      '`a %%b%% c`\n\nAfter.'
    );
  });

  it('drops file:// attachment links from the published text', () => {
    // The path resolves on the author's machine only, so publishing it gives
    // the post nothing and discloses the local username and folder layout.
    assert.equal(
      normalizeForPublishing(
  ...
  it('keeps file:// attachment links in the sidecar text', () => {
    // Same bake, other destination: the sidecar stays in the vault, where the
    // link resolves and is the point of the feature.
    const note = 'See this:\n\n![](file:///Users/someone/vault/report.pdf)\n\nEnd.';
    assert.equal(normalizeForPublishing(note), note);
  });

  it('drops a file:// link with alt text, inline and alone', () => {
    assert.equal(
      normalizeForPublishing('Before ![the doc](file:///a/b.pdf) after.', {
        stripLocalFileLinks: true,
      }),
      'Before after.'
    );
  ...
  it('never mistakes a remote image for a local file link', () => {
    // Only the `file://` form is generated by convertFileLinks; an http image
    // is the author's own content and must survive.
    const note = 'Look: ![](https://example.com/a.png)';
    assert.equal(
      normalizeForPublishing(note, { stripLocalFileLinks: true }),
      note
    );
  });

  it('keeps card boundaries intact when a marker is followed by prose', () => {
    // Regression guard: stripping comments before the structure markers would
    // consume the marker but leave its newline, adding a paragraph break.
    assert.equal(
      normalizeForPublishing('<!--section: 1-->\nOne.\n\n<!--section: 2-->\nTwo.'),
      'One.\n\nTwo.'
    );
  });
